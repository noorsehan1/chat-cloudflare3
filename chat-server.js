// ==================== CHAT-SERVER.JS ====================
// VERSION: 15.1.6 - PURGE ON RESTORE (NO LOG)
...
export class ChatServer {
  ...
  // ============================================================
  // 🔥🔥🔥 PURGE DEAD SESSIONS
  // Dipanggil saat server BANGUN (restore)
  // Hapus: memory + D1 by key + D1 by username + broadcast removeKursi
  // ============================================================
    ...
    try {
      ...
      for (const ws of liveWs) {
        try {
          ...
          if (typeof att.username !== 'string' || att.username.trim() === '') continue;
          liveUsernames.add(att.username);
        } catch(e) {}
      }

      // 2. Scan semua seat non-multi di memory
      const roomsData = this._storageCache?.roomsData || {};
      const toDelete = [];
      let totalNonMulti = 0;

      for (const [, roomBucket] of Object.entries(roomsData)) {
        if (!roomBucket?.seat) continue;
        for (const [, data] of Object.entries(roomBucket.seat)) {
          if (data?.namauser && data.isMulti !== true) totalNonMulti++;
        }
      }

      // 3. Kalau TIDAK ADA WS HIDUP → hapus SEMUA seat non-multi
      if (liveUsernames.size === 0 && totalNonMulti > 0) {
        for (const [roomName, roomBucket] of Object.entries(roomsData)) {
          if (!roomBucket?.seat) continue;
          for (const [seat, data] of Object.entries(roomBucket.seat)) {
        ...
        // 4. Ada WS hidup → hapus yang tidak ada di liveUsernames
        for (const [roomName, roomBucket] of Object.entries(roomsData)) {
          if (!roomBucket?.seat) continue;
          for (const [seat, data] of Object.entries(roomBucket.seat)) {
            if (!data?.namauser) continue;
      ...
      // 5. Hapus: memory + D1 by key + D1 by username + broadcast
      ...
      // 6. Bersihkan userConnections kosong
  ...
  async _restoreAllState() {
    try {
      ...
      // 🔥🔥🔥 HAPUS DATA YATIM SETELAH RESTORE
      ...
      // 🔥🔥🔥 Tetap coba purge walau restore gagal
...
export default ChatServer;// ==================== CHAT-SERVER.JS ====================
