...
export class GameServer {
  constructor(state, env) {
    ...
    this._quizForceRunning = false; // Flag untuk memaksa quiz tetap berjalan
  ...
  async _initAsync() {
    ...
    this._quizCheckInterval = setInterval(() => {
      ...
      // Jika ada user di room Quiz, pastikan quiz berjalan
      if (hasClients) {
        // Selalu coba jalankan quiz jika ada user di room
  ...
  _ensureQuizAlwaysRunning() {
    try {
      ...
      // Selalu set quizAutoEnabled ke true jika ada user di room Quiz
      this.quizAutoEnabled = true;
      this._quizForceRunning = true;
      
      // Jika quiz tidak sedang berjalan, force start
      if (!this.currentQuestion && !this._quizTimeout && !this.isQuizWaiting) {
        this.forceStartQuiz();
        
        // Jika masih belum jalan, coba lagi
      ...
      // Jika quiz sedang dalam break, percepat break
      if (this.isQuizWaiting && this._quizBreakTimeout) {
        // Biarkan break berjalan, tapi pastikan setelah break selesai quiz lanjut
      }
      ...
      if (isQuizTime) {
        ...
        // Meskipun tidak jam kuis, tetap jalankan quiz jika ada user
        this._broadcastToRoom(QUIZ_ROOM, [
          "quizTimeLeft",
  ...
  _getGameType(room) {
    if (room === QUIZ_ROOM) {
      // Quiz selalu aktif jika ada user di room
      return false;
    }
  ...
  _getTimeLeftMessage() {
    ...
    return `⏸️ Quiz is offline. Starts in ${parts.join(" ")}`;
  }
  ...
  async _checkQuizAutoStatus() {
    try {
      ...
      // Jika ada client di room Quiz, selalu aktifkan quiz
      if (hasClients) {
        ...
        // Jika quiz tidak berjalan, force start
        if (!this.currentQuestion && !this._quizTimeout && !this.isQuizWaiting && !this._quizStartTimeout) {
          await this._showQuestion();
        }
      } else {
        // Jika tidak ada client, tetap aktifkan quiz untuk berjaga-jaga
        // Tapi kita tidak perlu force start
        if (this.quizAutoEnabled && !this._isQuizTime()) {
          // Hanya matikan jika tidak ada client dan bukan jam kuis
          this.quizAutoEnabled = false;
  ...
  forceStartQuiz() {
    // Cek jika quiz sudah berjalan
    if (this.currentQuestion) {
      // Quiz already running, broadcast current question to new users
      this._broadcastCurrentQuestion();
      return true;
    }
    
    if (this._quizTimeout || this.isQuizWaiting) {
      // Quiz is in timeout or break, wait
      return false;
    }
  ...
  _checkAndRestartQuiz() {
    try {
      ...
      // Jika ada client, pastikan quiz berjalan
      if (hasClients) {
  ...
  ensureQuizRunning() {
    try {
      ...
      if (hasClients) {
        // Selalu aktifkan quiz jika ada client
        this.quizAutoEnabled = true;
        this._quizForceRunning = true;
        ...
        // Jika tidak ada client, tetap aktifkan untuk berjaga-jaga
        // Tapi jangan force start
  ...
  _sendQuizTimeLeftToUser(ws) {
    ...
    try {
      ...
      // Selalu kirim bahwa quiz sedang berjalan jika ada user di room
      ...
      // Jika quiz sedang berjalan, kirim pertanyaan yang sedang aktif
      if (this.currentQuestion) {
        this._sendCurrentQuestionToUser(ws);
      } else {
        // Jika belum ada pertanyaan, force start
        this.forceStartQuiz();
        setTimeout(() => {
          if (this.currentQuestion) {
            this._sendCurrentQuestionToUser(ws);
  ...
  _sendCurrentQuestionToUser(ws) {
    if (!ws || ws.readyState !== 1 || !this.currentQuestion) return;
  ...
  _sendQuizErrorWithTime(ws, errorType, customMessage = null) {
    ...
    try {
      ...
      // Jika ada error, tetap coba jalankan quiz
      this.forceStartQuiz();
    ...
    try {
      ...
      if (oldRoom === roomName) {
        if (roomName === QUIZ_ROOM) {
          // Selalu aktifkan quiz ketika masuk ke room Quiz
          this.quizAutoEnabled = true;
          this._quizForceRunning = true;
          ...
          // Force start quiz immediately
          this.forceStartQuiz();
          
          // Jika masih belum berjalan, coba lagi setelah delay
          if (!this.currentQuestion) {
            setTimeout(() => {
              if (!this.closing && !this.isDestroyed) {
                this.forceStartQuiz();
  ...
  async _showQuestion() {
    try {
      // Selalu tampilkan pertanyaan jika ada user di room Quiz
      const clients = this.wsClients.get(QUIZ_ROOM);
      const hasClients = clients && clients.size > 0;
      
      // Jika tidak ada client, tidak perlu menampilkan pertanyaan
      if (!hasClients) {
        return;
      }
      ...
      // Cek jika quiz sudah berjalan
      if (this.currentQuestion) {
        // Already showing a question, broadcast it to any new users
        this._broadcastCurrentQuestion();
        return;
      }
      ...
      this._quizTimeout = setTimeout(async () => {
        try {
          ...
          if (!currentClients || currentClients.size === 0) {
            this._quizTimeout = null;
            this.currentQuestion = null;
            // Jika tidak ada client, reset quiz
            this._quizForceRunning = false;
            return;
          }
          ...
          this._quizBreakTimeout = setTimeout(() => {
            ...
            if (!this.closing && !this.isDestroyed) {
              ...
              if (clients && clients.size > 0) {
                // Selalu lanjutkan quiz jika ada client
                this._showQuestion();
              } else {
                this._quizForceRunning = false;
              }
  ...
  async submitQuizAnswer(ws, username, answer) {
    try {
      ...
      // Selalu terima jawaban selama ada di room Quiz
      if (!this.quizAutoEnabled) {
        this.quizAutoEnabled = true;
        this._quizForceRunning = true;
        this.forceStartQuiz();
  ...
  _startQuizLoop() {
    ...
    this.quizTimer = setInterval(() => {
      ...
      // Selalu jalankan quiz jika ada client
      if (hasClients) {
        if (!this.quizAutoEnabled) {
          this.quizAutoEnabled = true;
          this._quizForceRunning = true;
        ...
        // Jika tidak ada client, biarkan quiz tetap siap
        // Tapi jangan force start
        if (this.quizAutoEnabled && !this._isQuizTime()) {
          this.quizAutoEnabled = false;
          this._quizForceRunning = false;
  ...
  async handleEvent(ws, data) {
    ...
    if (evt === "getQuizTimeStatus") {
      ...
      // Selalu kirim bahwa quiz aktif jika ada client
      this._safeSend(ws, ["quizTimeStatus", {
        isQuizTime: true,
        gameType: false,
...
}// ==================== GAME-SERVER.JS (FULL FIXED - AUTO START QUIZ ON SWITCH) ====================
...
export class GameServer {
  ...
  async _initAsync() {
    ...
    this._quizCheckInterval = setInterval(() => {
      ...
      if (clients && clients.size > 0) {
        // Jika ada user di room Quiz, pastikan quiz berjalan
        if (this._isQuizTime() && !this.currentQuestion && !this._quizTimeout && !this.isQuizWaiting) {
          this.forceStartQuiz();
        }
        // Jika tidak jam kuis, kirim status waktu
        if (!this._isQuizTime()) {
          this._broadcastToRoom(QUIZ_ROOM, [
            "quizTimeLeft",
    ...
    try {
      ...
      if (isQuizTime) {
        if (!this.quizAutoEnabled) {
          ...
          if (!this._quizStartTimeout && hasClients) {
            this.forceStartQuiz();
          }
        } else if (!this.currentQuestion && !this._quizTimeout && !this.isQuizWaiting && !this._quizStartTimeout && hasClients) {
          await this._showQuestion();
        } else if (!this.currentQuestion && hasClients) {
          // Jika ada clients tapi quiz belum jalan, force start
          this.forceStartQuiz();
        }
      } else {
        if (this.quizAutoEnabled) {
  ...
  _sendQuizTimeLeftToUser(ws) {
    ...
    try {
      ...
      // Jika quiz sedang berjalan, kirim pertanyaan yang sedang aktif
      if (isQuizTime && this.currentQuestion) {
        this._sendCurrentQuestionToUser(ws);
      }
    ...
    try {
      ...
      if (oldRoom === roomName) {
        if (roomName === QUIZ_ROOM) {
          ...
          // Pastikan quiz berjalan jika sudah jam kuis
          if (isQuizTime) {
            this.quizAutoEnabled = true;
            // Force start quiz immediately
            this.forceStartQuiz();
  ...
  _startQuizLoop() {
    ...
    this.quizTimer = setInterval(() => {
      ...
      if (this._isQuizTime()) {
        ...
        if (!this.currentQuestion && !this._quizTimeout && !this.isQuizWaiting && !this._quizStartTimeout && hasClients) {
          this._showQuestion();
        }
