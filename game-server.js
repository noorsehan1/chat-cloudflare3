...
export class GameServer {
  ...
  _addClient(room, ws, username = null, isNewConnection = false) {
    ...
    // Jika ws sudah di room yang sama, return saja
    if (ws.room === room) {
      return;
    }
    
    // Jika ws sudah punya room berbeda, pindahkan
  ...
  _getRoomForWs(ws) {
    // Cek dari ws.room terlebih dahulu
    if (ws && ws.room) {
      return ws.room;
    }
    const wsId = this._getWsId(ws);
  ...
  async switchRoom(ws, room, username = null) {
    ...
    // Cek apakah ws sudah di room yang benar
    const currentRoom = this.clientRooms.get(wsId) || ws.room;
    
    // Jika sudah di room yang sama, langsung kirim status
    if (currentRoom === roomName) {
      this._safeSend(ws, ["switchRoomSuccess", roomName]);
      
      // Kirim status game di room ini
      const game = this.activeGames.get(roomName);
      if (game && game._isActive && !game._gameEnded) {
        this._sendGameStatusToWs(ws, roomName);
      } else {
        // Kirim status bahwa tidak ada game aktif
    ...
    // Jika berbeda, pindahkan ke room baru
    if (currentRoom) {
  ...
  _cleanupGame(game) {
    ...
    const timers = ['_registrationTimer', '_drawTimer', '_safetyTimer'];
  ...
  _deleteGame(room, game) {
    ...
    // Reset game reference di ws tapi PERTAHANKAN client di room
    const wsIds = this.wsClients.get(room);
    if (wsIds) {
      for (const wsId of wsIds) {
        const ws = this.wsMap.get(wsId);
  ...
  async startGame(ws, bet, username) {
    try {
      ...
      // Ambil room dari ws atau clientRooms
      ...
      // Check dan cleanup existing game
...
export class GameServer {
  ...
  async startGame(ws, bet, username) {
    try {
      ...
      // Check and cleanup existing game
      const existingGame = this.activeGames.get(room);
      if (existingGame) {
        if (existingGame._gameEnded || !existingGame._isActive) {
          this._deleteGame(room, existingGame);
      ...
      try {
        ...
        // Clean room data before creating new game
        this._gameLocks.delete(room);
        this._joinLocks.delete(room);
        this.roomViewers.delete(room);
        ...
        if (wsIds) {
          for (const id of wsIds) {
            ...
            if (clientWs && clientWs !== ws) {
              try {
                clientWs.close(1000, "New game starting");
              } catch(e) {}
            }
          }
          this.wsClients.delete(room);
