...
class DataManager {
  ...
  async _ensureCacheInitialized(timeoutMs = 5000) {
    ...
    try {
      await Promise.race([this.loadAllData(), new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), timeoutMs))]);
    } catch(e) {
      this._cache = {
        recordingStatusMap: {},
        winnersMap: {},
  ...
  async _save(key, value) {
    try {
      ...
      await this.db.prepare(`INSERT OR REPLACE INTO ${TABLE_NAME} (key, value) VALUES (?, ?)`).bind(key, JSON.stringify(cleanValue)).run();
...
export class GameServer {
  ...
  _cleanupStaleTimers() {
    try {
      ...
      for (const timer of toRemove) this._clearTimer(timer);
    } catch(e) {}
  }
  ...
  _clearAllGameTimers(game) {
    if (!game) return;
    const timers = [game._regTimer, game._drawTimer, game._evalTimer, game._safetyTimer, game._startTimer];
    for (const timer of timers) this._clearTimer(timer);
    if (game._notifTimers) {
      for (const timer of game._notifTimers) this._clearTimer(timer);
      game._notifTimers = [];
    }
    if (game._drawNotifTimers) {
      for (const timer of game._drawNotifTimers) this._clearTimer(timer);
    ...
    try {
      ...
      if (oldRoom && oldRoom !== room) {
        ...
        if (oldClients) {
          ...
          if (oldClients.size === 0) this.wsClients.delete(oldRoom);
        }
      }
      ws.room = room;
      ws.roomname = room;
  ...
  _isGameActuallyRunning(game) {
    return game && game._isActive === true && !game._gameEnded && game.players && game.players.size > 0 && game._state !== this.GAME_STATE.ENDED && game._state !== this.GAME_STATE.IDLE;
  }

  _getActivePlayers(game) {
    try {
      if (!game || !game._isActive || game._gameEnded || !game.players) return [];
      return Array.from(game.players.entries()).filter(([id]) => !game.eliminated?.has(id)).map(([, p]) => p);
    } catch(e) { return []; }
  }
  ...
  async startGame(ws, bet, username) {
    try {
      ...
      this._ensureClientInRoom(ws, room, finalUsername);
      ...
      try {
        ...
        if (existing) this._forceCleanupGame(room, existing);
        
        if (finalBet < 0 || (finalBet > 0 && finalBet < 100) || finalBet > CONSTANTS.MAX_BET) {
        ...
        game.players.set(finalUsername, { id: finalUsername, name: finalUsername });
        if (wsId) game.playerWsId.set(finalUsername, wsId);
        this.activeGames.set(room, game);
        this._addClient(room, ws, finalUsername);
        this._broadcastToRoom(room, ["gameLowCardStart", finalBet]);
        this._broadcastToRoom(room, ["gameLowCardStartSuccess", finalUsername, finalBet]);
  ...
  _startRegistration(room, game) {
    try {
      ...
      const notifications = [{ delay: 5000, msg: "15s" }, { delay: 10000, msg: "10s" }, { delay: 15000, msg: "5s" }];
      for (const n of notifications) {
        const timer = setTimeout(() => {
          if (this._isGameActuallyRunning(game) && game._state === this.GAME_STATE.REGISTRATION) {
  ...
  _startDrawPhase(room, game) {
    try {
      ...
      for (const n of notifications) {
        const timer = setTimeout(() => {
          if (this._isGameActuallyRunning(game) && game._state === this.GAME_STATE.DRAW && !game.drawTimeExpired) {
            this._broadcastToRoom(room, ["gameLowCardTimeLeft", n.msg]);
          }
        }, n.delay);
        this._trackTimer(timer);
  ...
  _handleBotDraw(room, botId, game) {
    try {
      ...
      setTimeout(() => {
        if (!game.evaluationLocked && !game._isEvaluating && game._state === this.GAME_STATE.DRAW) {
    ...
    try {
      ...
      if (drawnCount === activeIds.length && activeIds.length > 1) {
        ...
        const evalTimer = setTimeout(() => {
          if (this._isGameActuallyRunning(game) && !game._gameEnded && game._state === this.GAME_STATE.DRAW) {
  ...
  async _evaluateRound(room, game) {
    ...
    if (!this._acquireLock(this._evalLocks, lockKey, CONSTANTS.EVALUATION_TIMEOUT_MS)) return;
    try {
  ...
  async joinGame(ws, username) {
    try {
      ...
      if (game.players.has(finalUsername)) {
        if (game.eliminated.has(finalUsername)) {
      ...
      if (wsId) game.playerWsId.set(finalUsername, wsId);
      this._addClient(room, ws, finalUsername);
      this._broadcastToRoom(room, ["gameLowCardJoin", finalUsername, game.betAmount]);
    } catch(e) {
  ...
  async submitNumber(ws, number, tanda, username) {
    try {
      ...
      if (game.numbers.has(finalUsername)) {
        this._safeSend(ws, ["gameLowCardError", "Already submitted"]);
        return;
      }
      const submitLockKey = `submit_${room}_${finalUsername}`;
      ...
      try {
        ...
        game.numbers.set(finalUsername, n);
        game.tanda.set(finalUsername, t);
        this._broadcastToRoom(room, ["gameLowCardPlayerDraw", finalUsername, n, t]);
        if (!game.evaluationLocked && !game._isEvaluating) {
          this._checkAllDrawn(room, game);
        }
      } finally {
  ...
  async switchRoom(ws, room, username = null) {
    try {
      ...
      if (Array.isArray(room)) {
        roomName = room.length > 0 ? room[0] : null;
        userName = room.length > 1 ? room[1] : null;
      }
      
      if (typeof roomName === 'string') {
        roomName = roomName.trim();
      } else if (roomName !== null && roomName !== undefined) {
      ...
      if (!userName || typeof userName !== 'string' || userName.trim() === '') {
        userName = "Player_" + wsId;
      }
      
      const finalUsername = userName.trim();
      const finalRoom = roomName;
      
      if (!finalRoom || finalRoom === "undefined" || finalRoom === "null" || finalRoom === "") {
      ...
      try {
        ...
        this._ensureClientInRoom(ws, finalRoom, finalUsername);
        
        // Update game state
        const newGame = this.activeGames.get(finalRoom);
        if (newGame && newGame._isActive && !newGame._gameEnded) {
          if (newGame.players.has(finalUsername)) {
            newGame.playerWsId.set(finalUsername, wsId);
          }
        ...
        // Send success
        this._safeSend(ws, ["switchRoomSuccess", finalRoom]);
        
        // Send room state
        const isRecording = await this.dataManager.getRecordingStatus(finalRoom);
        this._safeSend(ws, ["recordingStatus", isRecording]);
        
        // Send game status
        const game = this.activeGames.get(finalRoom);
        if (game && game._isActive && !game._gameEnded) {
  ...
  _startDiceFast() {
    try {
      ...
      try {
        ...
        this._diceTimeout = setTimeout(() => { this._endDiceRound(); }, 20000);
  ...
  async _runTieRound(room, id, players) {
    ...
    this.diceHasWinner = false;
    this.diceWinner = null;
    this._broadcastToRoom(room, ["diceNotification", `Tie Round ${this._tieRound}: ${players.join(', ')}`]);
    this._broadcastToRoom(room, ["diceTieBreaker", { round: this._tieRound, players: players }]);
  ...
  async _processTieResults(room, id, players) {
    ...
    if (answeredCount === 0) {
      this._broadcastToRoom(room, ["diceNotification", `No one answered in Round ${this._tieRound} - Tie breaker ended`]);
      this._broadcastToRoom(room, ["diceTieAnswer", { round: this._tieRound, results: [], winners: [], eliminated: players, status: 'no_answers' }]);
    ...
    if (answeredCount === 1) {
      ...
      this._broadcastToRoom(room, ["diceNotification", `${winner} answered with ${answer} - Auto win!`]);
      this._broadcastToRoom(room, ["diceTieAnswer", { round: this._tieRound, results: entries.map(e => `${e.player}:${e.answer}`), winners: [winner], eliminated: players.filter(p => p !== winner), status: 'single_winner' }]);
      try {
        const points = await this.dataManager.addDicePoint(winner);
        this._broadcastToRoom(room, ["diceWinner", { username: winner, totalPoints: points[winner] || 0, diceValue: answer, round: this._diceRound || 1, isTieBreaker: true, tieBreakerRound: this._tieRound, finalWinner: true, totalTieRounds: this._tieRound }]);
      } catch(e) {
        this._broadcastToRoom(room, ["diceWinner", { username: winner, totalPoints: 0, diceValue: answer, round: this._diceRound || 1, isTieBreaker: true, tieBreakerRound: this._tieRound, finalWinner: true, totalTieRounds: this._tieRound }]);
      }
      this._resetTieBreakerState(id);
      this._startCooldownAfterTieBreaker();
      return;
    ...
    if (allSame) {
      this._broadcastToRoom(room, ["diceNotification", `All answered same value: ${entries[0].answer} - Tie again!`]);
      const allPlayers = entries.map(e => e.player);
      this._broadcastToRoom(room, ["diceTieAnswer", { round: this._tieRound, results: entries.map(e => `${e.player}:${e.answer}`), winners: [], eliminated: [], status: 'all_same', allPlayers: allPlayers, value: entries[0].answer }]);
    ...
    if (highestPlayers.length === 1) {
      ...
      this._broadcastToRoom(room, ["diceNotification", `${winner} wins with highest value: ${highest}`]);
      this._broadcastToRoom(room, ["diceTieAnswer", { round: this._tieRound, results: entries.map(e => `${e.player}:${e.answer}`), winners: [winner], eliminated: losers, status: 'winner_found' }]);
      try {
        const points = await this.dataManager.addDicePoint(winner);
        this._broadcastToRoom(room, ["diceWinner", { username: winner, totalPoints: points[winner] || 0, diceValue: highest, round: this._diceRound || 1, isTieBreaker: true, tieBreakerRound: this._tieRound, finalWinner: true, totalTieRounds: this._tieRound }]);
      } catch(e) {
        this._broadcastToRoom(room, ["diceWinner", { username: winner, totalPoints: 0, diceValue: highest, round: this._diceRound || 1, isTieBreaker: true, tieBreakerRound: this._tieRound, finalWinner: true, totalTieRounds: this._tieRound }]);
      }
      this._resetTieBreakerState(id);
      this._startCooldownAfterTieBreaker();
      return;
    ...
    if (highestPlayers.length > 1) {
      this._broadcastToRoom(room, ["diceNotification", `Tie again! Round ${this._tieRound + 1} between: ${highestPlayers.join(', ')}`]);
      this._broadcastToRoom(room, ["diceTieAnswer", { round: this._tieRound, results: entries.map(e => `${e.player}:${e.answer}`), winners: [], eliminated: players.filter(p => !highestPlayers.includes(p)), status: 'tie_continue', remainingPlayers: highestPlayers, nextRound: this._tieRound + 1 }]);
  ...
  async _processSingleWinner(room, id, winner) {
    ...
    try {
      const points = await this.dataManager.addDicePoint(winner);
      this._broadcastToRoom(room, ["diceWinner", { username: winner, totalPoints: points[winner] || 0, diceValue: 'auto', round: this._diceRound || 1, isTieBreaker: true, tieBreakerRound: this._tieRound, finalWinner: true, totalTieRounds: this._tieRound }]);
    } catch(e) {
      this._broadcastToRoom(room, ["diceWinner", { username: winner, totalPoints: 0, diceValue: 'auto', round: this._diceRound || 1, isTieBreaker: true, tieBreakerRound: this._tieRound, finalWinner: true, totalTieRounds: this._tieRound }]);
    }
    this._resetTieBreakerState(id);
    this._startCooldownAfterTieBreaker();
  }
  ...
  async _handleEventInternal(ws, data) {
    try {
      ...
      if (evt === "switchRoom") {
        ...
        await this.switchRoom(ws, roomParam, usernameParam);
        return;
      }
      
      if (evt === "checkGameRunning") {
        this._checkGameRunning(ws, data.length > 1 ? data[1] : null);
        return;
      }
      ...
      if (evt === "getRoomWinners") {
        const roomName = data.length > 1 ? data[1]?.trim() : this._getClientRoom(ws);
        if (!roomName) {
          this._safeSend(ws, ["recordingError", "Room name required"]);
          return;
        }
        ...
        this._safeSend(ws, ["roomWinners", { winners: winners || {}, room: roomName, recording: isRecording || false }]);
        return;
      }
      
      if (evt === "addLowCardWinner") {
        const room = data.length > 1 ? data[1] : null;
        const username = data.length > 2 ? data[2] : null;
        if (!room || !username) {
      ...
      if (evt === "gameLowCardNumber") {
        const number = data.length > 1 ? data[1] : 0;
        const tanda = data.length > 2 ? data[2] : "";
        const username = data.length > 3 ? data[3] : null;
  ...
  _checkGameRunning(ws, roomname) {
    try {
      let room = roomname || this._getClientRoom(ws);
      if (!room) {
        this._safeSend(ws, ["gameStatus", "false"]);
        return;
      }
  ...
  async webSocketClose(ws, code, reason, wasClean) {
    try {
      ...
      if (ws) {
        ...
        this.wsMap.delete(wsId);
        this.clientRooms.delete(wsId);
        if (ws.username) this.userConnections.delete(ws.username);
      }
    }
  }
